...
const CUURENT_DIR = dirname(fileURLToPath(import.meta.url))
...
const createMulterConfig = (destinationPath, subFolder) => {
    return multer ({
        storage: multer.diskStorage({
            destination: join(CUURENT_DIR, destinationPath),
            filename: (req, file, cb) => {
                const fileExtension = extname(file.originalname)
                const fileName = file.originalname.split(fileExtension)[0, 8]
        ...
        fileFilter: (req, file, cb) => {
            ...
            else cb(new Error('Tipo de archvo no permitido'))    
        },
        limits: {
